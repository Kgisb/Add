...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        ...
        "Marketing": ["Referrals","HubSpot Deal Score tracker", "Deal Score Trend","Marketing Lead Performance & Requirement","Kids detail", "Deal Detail", "Sales Intern Funnel", "Master analysis", "Referral Tracking", "Talk Time", "referral_Sibling", "Deal Score Threshold", "Invalid Deals"],
    }
...
def _render_marketing_deal_score_trend(df):
    ...
    if group_by != "(Overall)":
        # Build value list AFTER we parse dates, so move this to after date filter
        pass
    # Date presets
    ...
    # Populate dependent values and filter if applicable
    if group_by != "(Overall)":
    ...
    if group_by == "(Overall)":
        ...
        key = group_by if group_by in dfx.columns else None
        if key is None:
            st.error(f"Group column not found: {group_by}")
            return
        dfx["_g"] = dfx[key].fillna("(Blank)")

    # Aggregate: mean by default for scores (you can switch to sum via a toggle if needed)
    agg_mode = st.radio("Aggregate as", ["Mean","Median"], horizontal=True, index=0)
    fun = np.nanmean if agg_mode=="Mean" else np.nanmedian
    ...
    if slider_candidates:
        ...
        with csl2:
            rng = st.slider(f"{slider_var_label} range", min_value=float(_min), max_value=float(_max), value=(float(_min), float(_max)), step=float(max((_max - _min)/100.0, 0.1)))
        # Apply filter
    ...
    for label in vars_sel:
        ...
        if col not in dfx.columns:
            continue
        grp = dfx.groupby(["_t","_g"], as_index=False)[col].agg(fun)
        grp["Variable"] = label
    ...
    st.caption("Note: Averages/medians computed over deals per time bucket.")

# Dispatch: if current section matches, render the pill
try:
    if master == "Marketing" and st.session_state.get('nav_sub') == "Deal Score Trend":
        # Prefer filtered dataframe if available, else raw df
        _df_any = None
...
def _render_marketing_deal_score_threshold(df):
    ...
    # Candidate date columns (in order of preference)
    date_options = [c for c in [
        "Payment Received Date", "Last Activity Date", "Create Date"
    ] if c in df.columns]
    if not date_options:
    ...
    # Filter by date per MTD/Cohort
    dfx = df.copy()
    if mode == "MTD":
        # Event inside window AND Create Date inside window
        ...
        # Cohort: Event inside window; Create anywhere
        dfx = dfx[(dfx[event_date]>=start) & (dfx[event_date]<=end)]
    ...
    # Select threshold classes to include
    ...
    # Aggregate counts by time and threshold (and group)
    ...
    if group_by != "(Overall)":
        chart = enc.encode(row=alt.Row("_g:N"))
    else:
        chart = enc
...
def _render_marketing_invalid_deals(df):
    ...
    st.caption("Counts and distribution for any **Deal Stage** containing the word **invalid** (case-insensitive).")

    # Try to ensure we use the raw dataframe (some global views exclude invalids)
    # If this function received a filtered df, attempt to reload original if needed.
    df_in = df.copy()
    ...
    # Filter to rows whose stage contains 'invalid' (case-insensitive)
    mask_invalid = df_in[deal_stage_col].astype(str).str.contains(r"invalid", case=False, na=False)
    dfx = df_in[mask_invalid].copy()

    # Early exit if none
    if dfx.empty:
        st.info("No rows where Deal Stage contains 'invalid'.")
        return
    ...
    # Deal Stage selector (only invalid-containing stages in current window)
    _stages = sorted([str(x) for x in dfx[deal_stage_col].dropna().unique().tolist()])
    if not _stages:
        st.info("No invalid stages to select.")
        return
    sel_stages = st.multiselect("Deal Stage (select one or more)", _stages, default=_stages)
    if sel_stages:
        dfx = dfx[dfx[deal_stage_col].astype(str).isin(sel_stages)]
    else:
        st.info("No stages selected — showing nothing.")
        dfx = dfx.iloc[0:0]

    if dfx.empty:
        st.info("No data after deal stage filter.")
        return
    # KPI total
    st.metric("Invalid deals (after filters)", int(len(dfx)))

    # Choose distribution basis: exact stage string among invalids
    dfx["_stage_invalid"] = dfx[deal_stage_col].astype(str)
    ...
    # Aggregate counts
    grp = dfx.groupby(["_t","_g","_stage_invalid"], as_index=False).size().rename(columns={"size":"Count"})
    grp = grp.sort_values(["_t","_g","_stage_invalid"])
    ...
    if chart_type == "Line":
        ...
        enc = base.encode(
            x=alt.X("_t:T", title="Date"),
            y=alt.Y("Count:Q"),
            color=alt.Color("_stage_invalid:N", title="Invalid Stage"),
            tooltip=["_t:T","_g:N","_stage_invalid:N","Count:Q"]
        )
    elif chart_type == "Stacked Bar":
        base = alt.Chart(grp).mark_bar()
        enc = base.encode(
    ...
    # Table + Download
    st.markdown("#### Data")
    st.dataframe(grp.rename(columns={"_t":"Date","_g":group_by,"_stage_invalid":"Invalid Stage"}), use_container_width=True)
    st.download_button(
        "Download CSV — Invalid Deals",
        grp.rename(columns={"_t":"Date","_g":group_by,"_stage_invalid":"Invalid Stage"}).to_csv(index=False).encode("utf-8"),
        "invalid_deals.csv",
        "text/csv"
    )
    ...
    if master == "Marketing" and st.session_state.get('nav_sub') == "Invalid Deals":
        ...
        for _nm in ("df","dff","df_f"):
            ...
            if _cand is not None:
                ...
                break
        if _df_any is None:
            # As a fallback, try to reload from session data_src
            import streamlit as st, pandas as pd
            _path = st.session_state.get("data_src") or globals().get("DEFAULT_DATA_PATH", "Master_sheet-DB.csv")
            try:
                _df_any = pd.read_csv(_path)
            except Exception:
                _df_any = None
